...
function Jewelery  ()  {
    const {productJeweleryApiFunction} = useApi([]);

    useEffect(()=>{
        productJeweleryApiFunction();
    },[]);
    const token = localStorage.getItem("token");
  const productData = JSON.parse(localStorage.getItem("Jewelery"));
  console.log("---- random Jewelery product-------", productData);
  return (
    <>

    {token ? ( // Check if a valid token exists
      <div className="mainDiv">
        <h1 className="text-center py-4">ALL Jewelery's PRODUCTS</h1>
