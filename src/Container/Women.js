...
function Women() {
  ...
  const { productWomenApiFunction } = useApi([]);
  // use the function by the hook
  useEffect(() => {
    productWomenApiFunction();
  }, []);
  const token = localStorage.getItem("token");
  const productData = JSON.parse(localStorage.getItem("Womensdata"));
  console.log("---- random women product-------", productData);
  return (
    <>

      {token ? ( // Check if a valid token exists
        <div className="mainDiv">
          <h1 className="text-center py-4">ALL Women's PRODUCTS</h1>
